...
public class UserServiceTest {
   ...
   * <p>
   * 3. Then (검증: 이런 결과가 나와야 한다) 핵심 검증 (Argument Captor): UserRepository의 save() 메서드가 호출될 때, 그 메서드에
   * **매개변수로 넘겨진 유저 객체(User)**를 몰래 낚아챕니다(Capture). UserRepository.save()가 호출될 때 캡처한 User 객체의 비밀번호를
   * 확인합니다.
   * <p>
  ...
  public void 비밀번호가_암호화되어_저장된다() {
    ...
    // given
    RegisterCommand command = new RegisterCommand("test1@test.com", "password123", "이름1", "닉네임1");
